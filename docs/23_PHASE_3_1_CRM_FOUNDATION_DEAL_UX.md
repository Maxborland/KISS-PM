...
- товары и услуги являются отдельной CRM-сущностью каталога, а не полем сделки;
...
Tenant admin с правом `tenant.products.manage` может создать, редактировать и архивировать товар или услугу. Товар/услуга имеет:
...
Чтение требует `tenant.products.read`. Все операции tenant-scoped и пишут audit.

UI-владелец сущности: отдельная страница `Товары и услуги` (`/products`). В текущем baseline связь товара со сделкой не показывается как действие, пока нет persisted модели состава сделки `DealLineItem`.
...
- `Файлы` — persisted metadata/link для файлов без binary storage в текущем slice;
- raw audit evidence остается backend/read-model частью ответа и показывается только там, где есть отдельный реальный UI-сценарий и право `tenant.audit_events.read`.

Глобальный `/audit` остается отдельным административным evidence layer. Это не отменяет необходимость показывать operator-facing историю конкретной сделки внутри `/opportunities/:id`: пользователи с `tenant.opportunities.read` видят безопасную ленту системных событий без raw audit payload.

Activity model больше не является deal-only: общий `CrmActivity` привязывает комментарии, follow-up задачи и файловые ссылки к `opportunity`, `client`, `contact` и `product`. Карточки клиента, контакта и товара используют тот же правый workspace, но с собственными permission keys.
...
- AC3.1 -> DB/API tests: create/list/update product, restricted `403`, audit.
...
- AC6 activity -> API/Web/E2E: deal detail has activity panel with persisted comment creation, deal follow-up task creation/completion, file metadata links and same-opportunity system events; client/contact/product detail pages use the same persisted `CrmActivity` contract instead of placeholders.
