...
- [x] **`/admin` лендинг** → редирект на `/admin/users` (минус один консьюмер монолита).
- [x] **`/admin/audit`** → построена v3-поверхность `AdminAuditSurface` (вкладка «Аудит» в AdminFrame): боевой
