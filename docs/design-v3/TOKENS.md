...
Этот документ — единственный источник истины по тому, какой токен побеждает
в каждой коллизии и какие токены тащим дополнительно из планирования.
...
| Token | design-v2 | planning-ui-approved | Winner | Файл |
|---|---|---|---|---|
| `--accent` | `#2563eb` (blue) | `#0f172a` (dark) | **design-v2** ✅ Решение №2 | `tokens.css` |
| `--accent-hover` | `#1d4ed8` | _(нет)_ | design-v2 | `tokens.css` |
| `--accent-soft` | `#eef4ff` | `#1d4ed8` (другая семантика) | **design-v2** | `tokens.css` |
| `--accent-muted` | `#dbeafe` | _(нет)_ | design-v2 | `tokens.css` |
| `--accent-ring` | `rgba(37,99,235,0.18)` | _(нет)_ | design-v2 | `tokens.css` |
...
- Размеры: `--text-xs..4xl` = 11 / 12 / 13 / 14 / 16 / 20 / 26 / 32 / 40.
- Line-heights: `--lh-xs..4xl` = 16 / 18 / 20 / 22 / 24 / 28 / 32 / 38 / 44.
