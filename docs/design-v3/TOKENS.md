...
по фактически отгруженным значениям** — `apps/web/src/styles/tokens.css`
(единственный владелец `:root`; нормативный контракт — корневой `DESIGN.md`) и
машинный гейт `apps/web/src/__health__/design-v3-enforcement.health.test.ts`.

> **ОБНОВЛЕНО 2026-07-12 — унификация владельца.** `kiss-v4.css` больше не
> определяет `:root`: индиго-канон перенесён в `tokens.css` 1:1 (parity
> подтверждён картой каскада), `kiss-v4.css` — только утилитарные классы.
> Примечание к пункту «Dark OUT» ниже: с 2026-07 в проде есть МИНИМАЛЬНАЯ
> тёмная карта поверхностей/текста (`[data-theme="dark"]` в globals.css,
> активируется из профиля); полная семантическая dark-карта — отдельная работа.
...
| `--touch-target` | `44px` | _(нет)_ | Минимальная интерактивная область при `pointer: coarse` |
