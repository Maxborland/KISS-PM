...
- Desktop sidebar поддерживает compact rail с доступными названиями разделов, а narrow viewport использует offcanvas drawer: закрытое меню inert, открытое меню держит focus trap, `Escape` закрывает drawer, а закрытие или выбор раздела возвращают фокус на кнопку открытия навигации.
...
7. Расширить DB/API tests и browser smoke:
   ...
   - inline validation, keyboard focus baseline и destructive confirmation в CRUD-модалках.
