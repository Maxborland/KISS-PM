...
|---|---|---|---|---|
| **0. Docs-контракт + фиксы gate** | Утвердить контракт эпика; снять дешёвые блокеры, валящие любую comms-сторю | `docs/46` (backend media plane), `docs/47` (UI-контракт), правки `docs/api/07/99/00`, `docs/12`, `README`, beta-матрицы (как `deferred`), hard-fail policy продакшна. Фикс `UI_ONLY_PREVIEW_BANNER_TEXT` → «Превью — бэкенд не подключён», добавить `chat/calls/meetings/notifications` в `UI_ONLY_PREVIEW_SURFACES`. **Разрешить коллизию миграции 0041** (оба backend-слайса претендуют на 0041 → единый `0041`, CallEgress сворачивается в `call_recording_groups`). | M | both |
