...
- ◐ **P0-2 · Гантт — статичная таблица.** ЧАСТИЧНО: ✅ % complete теперь рисуется в баре (`.gbar__progress`, UX-008). ОСТАЁТСЯ (всё — визуал/интеракция, требует живого приложения для честной проверки; стек завязан на migration-reconcile второго Claude): **drag-move + resize-handle** (preview→apply), **стрелки зависимостей** (портировать `GanttDependencyArrows` — пиксельная привязка якорей проверяема только визуально), **колонка/tooltip «Резерв, дн.»** (UX-009: per-task `totalSlackMinutes` не проброшен в read-model фронта — сперва подтвердить сериализацию на живом API).
