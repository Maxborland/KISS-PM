...
- ☑ **P0-6 · Нет виртуализации задач — СДЕЛАНО.** Стрелки вынесены в единый overlay-слой (`.gantt2__deps`, геометрия через ref+ResizeObserver), поэтому `content-visibility:auto`+`contain-intrinsic-size` на строках даёт нативную виртуализацию (браузер пропускает off-screen строки), НЕ разрывая стрелки. Эмпирически подтверждено headless-скриншотом: сначала доказал, что content-visibility без overlay режет стрелки (paint-контейнмент), затем — что overlay это устраняет (7 стрелок + прогресс + сворачивание рендерятся корректно с виртуализацией). Пагинация плоского `TaskTable` (`GET …/tasks`) — отдельный безопасный под-пункт, не нужен Гантту.
  <br>Историческая заметка (почему сразу не сделал): ниже сохранена исходная оценка сложности — Причины конкретные, не «не успел»: (1) **Конфликт с UX-006**: стрелки зависимостей рисуются в ячейке строки-преемника и тянутся ВВЕРХ к строке-предшественнику; при виртуализации (строка-предшественник вне окна) стрелка рвётся — нужен отдельный always-present overlay-слой для стрелок, т.е. редизайн, а не «обернуть в react-window». (2) **Раскладка**: виджет — CSS-grid со «липкими» левыми колонками и горизонтальным скроллом; виртуализация требует фиксированных высот строк и переноса позиционирования — существенный риск сломать выравнивание, проверяемый ТОЛЬКО визуально. (3) **Выгода — только на масштабе** (тысячи задач); для реальных проектов (десятки–сотни задач) текущий Гантт работает. Делать в отдельном заходе с живым приложением + решением по overlay-слою стрелок. Пагинация плоского `TaskTable` (`GET …/tasks` limit/offset) — безопасный независимый под-пункт, но это контракт-изменение, проверяемое db-тестами/живым API.
...
- ☑ **P0-2 · Гантт — статичная таблица.** ЗАКРЫТ: ✅ % complete в баре (UX-008); ✅ **стрелки зависимостей FS** (`computeDependencyArrows` + SVG, геометрия в тестах); ✅ **drag бара по времени** (`dayDeltaFromDrag` в тестах, `task.update_schedule`); ✅ **resize правого края** (меняет длительность, защита «не короче 1 дня»); ✅ **резерв (slack) в tooltip строки** (UX-009: `totalSlackMinutes` проброшен из read-model в минутах→дни). Всё за permission-гейтом, сводные/вехи не двигаются. Визуальная плавность жестов + пиксель-полировка стрелок — финальный live-QA (Storybook на :6006 компилируется без ошибок).
