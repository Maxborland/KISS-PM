...
Свернуто: только иконки.
Раскрыто: узкая панель с подписями.
Пункты меню:
...
Генри переходит в состояние thinking.
Показывается живая последовательность действий:
Читает задачи
Проверяет сроки
Смотрит зависимости
Сверяет загрузку
...
Кнопка кратко показывает Применяем....
Выбранные изменения получают статус применено.
Внизу панели появляется audit strip:
