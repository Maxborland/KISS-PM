...
| Planning / Gantt | `/projects/:id/timeline` | PM-03 | wired/stronger | Read-model real; preview/apply с planVersion доказан; stale-conflict UI и richer direct manipulation остаются gaps | `pnpm test:e2e:smoke` |
...
| Deals pipeline | `/deals` | CEO-02, SALES-02 | wired | Нужны richer next actions and no-results states | route smoke + business smoke |
| Deal detail / handoff | `/deals/:id` | SALES-03, FIN-01 | wired | Route id real; нужна fuller handoff/activation evidence after reload | route smoke |
| Clients | hidden | SALES-01 | deferred | Runtime route intentionally absent from beta navigation | нет |
| Contacts | hidden | SALES-01 | deferred | Runtime route intentionally absent from beta navigation | нет |
...
- `/projects/:id`, `/projects/:id/timeline`, `/projects/:id/resources`, and `/deals/:id` use real route ids and invalid-id checks; project task create/comment/blocker and planning preview/apply have persistence proof.
