...
- `REF-003` in_progress: shell-state, route icon registry, dashboard presentation, users CRUD view, roles CRUD view, positions CRUD view и audit view вынесены из `App.tsx`; нужны page/view-level slices до бюджета меньше 500 строк.
...
1. `App.tsx`: вынести `WorkspaceSettingsView`, `ProfileView`, `ThemeView` по одному; где возможно, привязать к Next.js route page boundaries.
