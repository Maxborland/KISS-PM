...
## Release 2 — UI control surface hardening

Release 2 is a cross-phase UI/product hardening release, so it uses the `E2E-R2-*` id family instead of renumbering existing phase scenarios. These scenarios prove that key P3-P10 surfaces behave as KISS PM management instruments: data -> signal -> governed action -> preview/result -> audit -> refreshed projection -> reload/readback proof.

Product-app reset blocker: `E2E-R2-001..010` are not sufficient app-level SaaS readiness evidence when they do not prove real routes/pages/session/profile/settings/demo seed/readback. Release 2 implementation is blocked until `E2E-R2R-001 Product owner smoke` passes through the reset contract.

| ID | Scenario | Role | Fixture | Preconditions / expected result | Domain modules | PRD refs | Test path | Status |
|---|---|---|---|---|---|---|---|
| E2E-R2-001 | Portfolio Control: signal to governed action to audit/readback | Executive / Project manager | Seed Tenant A with active project signals | User opens Portfolio Control, drills into the highest-risk signal, executes a permitted governed action, sees ActionExecution/AuditEvent evidence, and reload keeps the handled/refreshed state | control-surfaces, action-engine, audit, project-core, kpi-engine | PRD 10.9, 10.10 | `e2e/tests/release2/portfolio-control.spec.ts` | passing |
| E2E-R2-002 | Project Gantt: Excel-like edit persists, audit appears, reload keeps state | Project manager | Seed Tenant A active project with WBS/tasks | User edits an active Gantt grid cell, sees validation/dirty/pending state, saves through API/action layer, sees audit/result readback, and reload keeps the changed dates | scheduling-engine, project-core, action-engine, audit | PRD 10.6, 10.10 | `e2e/tests/release2/gantt-planning.spec.ts` | passing |
| E2E-R2-003 | Project Gantt tracking: baseline remains stable, live changes show variance | Project manager | Seed Tenant A active project with baseline | User toggles tracking, changes live plan, sees baseline bars remain stable and live-vs-baseline variance persist after reload | scheduling-engine, audit | PRD 10.6 | `e2e/tests/release2/gantt-planning.spec.ts` | passing |
| E2E-R2-004 | Resource Capacity Matrix: overload cell opens affected work and resolution flow | Resource manager | Seed Tenant A overloaded resource period | User opens Resource Load, focuses overload cell, opens drilldown, sees affected work/source refs and resolution entrypoints with disabled reasons where relevant | resource-planning, scheduling-engine, control-surfaces | PRD 10.7, 10.9 | `e2e/tests/release2/resource-capacity.spec.ts` | passing |
| E2E-R2-005 | Resource conflict resolution: preview shift/split/reassign, apply, audit, reload | Resource manager | Seed Tenant A overloaded resource period | User previews a resource resolution, sees before/after load, blockers/warnings, confirms when allowed, sees ActionExecution/AuditEvent and recalculated load after reload | resource-planning, scheduling-engine, action-engine, audit | PRD 10.7, 10.10 | `e2e/tests/release2/resource-capacity.spec.ts` | passing |
| E2E-R2-006 | KPI Deviation: source trace to corrective action or accepted risk | Executive / Project manager | Seed Tenant A KPI deviation | User opens deviation, inspects source/formula/threshold trace, creates corrective action or accepts risk with reason, sees audit/result and handled status after reload | kpi-engine, control-surfaces, action-engine, project-core, audit | PRD 10.8, 10.9, 10.10 | `e2e/tests/release2/portfolio-control.spec.ts` | passing |
| E2E-R2-007 | Closed Portfolio: snapshot/trend to template-improvement action | Executive / Tenant admin | Seed Tenant A closed snapshots | User opens closed portfolio/trends, compares current vs previous period, previews template-improvement action, confirms it, and verifies snapshot immutability plus future-template readback | retrospective, project-core, tenant-config, action-engine, audit | PRD 10.11, 10.10 | `e2e/tests/release2/retrospective.spec.ts` | passing |
| E2E-R2-008 | Tenant Admin layout/saved view affects runtime surface after reload | Tenant admin | Seed Tenant A saved-view/config fixture | Admin changes a saved view or layout with preview, publishes versioned config, opens affected runtime surface, reloads, and sees the new layout with audit evidence | tenant-config, control-surfaces, audit | PRD 10.1, 10.9, 10.10 | `e2e/tests/release2/tenant-admin-config.spec.ts` | passing |
| E2E-R2-009 | Read-only user sees disabled reasons and backend denies mutation | Read-only observer | Seed Tenant A read-only role across R2 surfaces | Read-only user can inspect allowed context, sees disabled reasons for mutations, and a direct mutation attempt is denied by backend/action path without state change | access-control, action-engine, control-surfaces, audit | PRD 10.2, 10.10 | `e2e/tests/release2/tenant-admin-config.spec.ts` | passing |
| E2E-R2-010 | Sales demo path: first 5 minutes surface clarity and no dead-end screen | Demo PM / Executive | Seed Tenant A demo portfolio | Viewer can identify workspace, object, risk, and next action in key R2 surfaces within five minutes; no visited screen is passive or dead-ended | web, control-surfaces, scheduling-engine, resource-planning, kpi-engine | PRD 8.1, 10.9, 14 | `e2e/tests/release2/sales-demo-first-five-minutes.spec.ts` | passing |

## Release 2 App Foundation Reset

| ID | Scenario | Role | Fixture | Preconditions / expected result | Domain modules | PRD refs | Test path | Status |
|---|---|---|---|---|---|---|---|---|
| E2E-R2R-001 | Product owner smoke | Product owner / tenant admin / project manager / resource manager / read-only observer | Seed Tenant A with resettable demo data | User can login/select dev user, land on role dashboard, navigate real pages, open profile/account, open admin settings, configure KPI threshold through preview, publish/readback/audit, open seeded project Gantt without manual API seeding, inspect resource/portfolio signal, prove reload/deep link/back-forward, and prove read-only backend denial | web, access-control, tenant-config, scheduling-engine, resource-planning, kpi-engine, control-surfaces, action-engine, audit | PRD 8.1, 10.1, 10.2, 10.6, 10.7, 10.8, 10.9, 10.10 | `e2e/tests/release2-app-foundation/product-owner-smoke.spec.ts` | planned |
