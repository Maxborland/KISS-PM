...
const artifactDir = path.join(root, "docs", "design-mockups", "artifacts", "project-gantt-v5");
...
await page.evaluate(() => localStorage.removeItem("kisspm-gantt-mock-v5-state"));
await page.reload({ waitUntil: "domcontentloaded" });

await page.screenshot({ path: path.join(artifactDir, "desktop-initial.png"), fullPage: false });

await pass("BROWSER-001", "page opens as Project Gantt v5", async () => (await page.title()).includes("v5"));
...
const report = {
  screen: "project-gantt-planner-v5",
  url,
  checked_at: new Date().toISOString(),
  artifacts: {
    desktop_initial: "docs/design-mockups/artifacts/project-gantt-v5/desktop-initial.png",
    desktop_after_apply: "docs/design-mockups/artifacts/project-gantt-v5/desktop-after-apply.png",
    narrow_1024: "docs/design-mockups/artifacts/project-gantt-v5/narrow-1024.png"
  },
...
console.log(JSON.stringify({ checked_at: report.checked_at, failed: failed.length, rows: rows.length, artifact: "docs/design-mockups/artifacts/project-gantt-v5/browser-verification-report.json" }, null, 2));
if (failed.length > 0) {
  process.exitCode = 1;
}
