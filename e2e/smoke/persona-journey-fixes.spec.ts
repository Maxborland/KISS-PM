...
test("новый владелец проходит первый рабочий цикл и получает честные квитанции", async ({ page }) => {
  ...
  // поэтому агент отвечает по-настоящему. Финал хода ждём по СТАБИЛЬНОМУ сигналу
  // (testid реплики агента), а не по копирайту scripted-провайдера — его строки
  // не продуктовый контракт и дрейфуют. У свежего владельца ровно одна реплика.
  // Ветка «LLM-провайдер не настроен» покрыта unit-тестами (503) и Storybook.
  await expect(page.locator('[data-testid="agent-turn"][data-turn-role="agent"]')).toHaveCount(1, { timeout: 15_000 });
