...
test("новый владелец проходит первый рабочий цикл и получает честные квитанции", async ({ page }) => {
  ...
  // финальный ответ «действий не найдено». Ассертим финальную реплику, а не префикс
  // live-trace («смотрю ваши задачи»), чтобы не уходить со страницы посреди stream.
  // Ветка «LLM-провайдер не настроен» покрыта unit-тестами (503) и Storybook.
  await expect(page.getByRole("log")).toContainText("подходящих действий не найдено");
