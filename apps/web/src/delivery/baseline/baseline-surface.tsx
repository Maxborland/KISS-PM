...
export function ProjectBaseline() {
  ...
  if (status === "loading" && !readModel) {
    return <DeliveryFrame project={PROJECT_FALLBACK} activeTab="Baseline"><div className="flex h-[420px] items-center justify-center gap-2 rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--panel)] text-[var(--muted)]"><Loader2 className="size-4 animate-spin" aria-hidden /> Загрузка…</div></DeliveryFrame>;
  }
  if (status === "error" || !model || !readModel) {
    return <DeliveryFrame project={PROJECT_FALLBACK} activeTab="Baseline"><div className="flex h-[420px] flex-col items-center justify-center gap-3 rounded-[var(--radius-card)] border border-[var(--danger)] bg-[var(--danger-soft)] text-[var(--danger-text)]"><span>Не удалось загрузить: {error ?? "unknown"}</span><Button variant="secondary" size="sm" onClick={() => void reload()}>Повторить</Button></div></DeliveryFrame>;
  }
