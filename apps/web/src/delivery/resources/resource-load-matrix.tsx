...
export function ResourceLoadMatrix({ scope, data, callbacks = {} }: { scope: MatrixScope; data: MatrixData; callbacks?: MatrixCallbacks }) {
  ...
  const leftRow = (r: Rrow, opts?: { totals?: boolean }) => {
    ...
    return (
      <div key={r.key} className={cn("flex items-center gap-1.5 border-b px-2", opts?.totals ? "border-[var(--border-strong)] bg-[var(--panel-subtle)]" : "border-[var(--border-subtle)]", !opts?.totals && (r.depth === 0 ? "bg-[color-mix(in_oklab,var(--panel-strong)_55%,var(--panel))]" : r.depth === 1 && "bg-[color-mix(in_oklab,var(--panel-strong)_25%,var(--panel))]"))} style={{ height: ROW_H, width: LEFT_W }}>
  ...
  const periodRow = (r: Rrow, opts?: { totals?: boolean }) => (
    <div key={r.key} className={cn("flex border-b", opts?.totals ? "border-[var(--border-strong)] bg-[var(--panel-subtle)]" : "border-[var(--border-subtle)]", !opts?.totals && (r.depth === 0 ? "bg-[color-mix(in_oklab,var(--panel-strong)_40%,var(--panel))]" : r.depth === 1 && "bg-[color-mix(in_oklab,var(--panel-strong)_18%,var(--panel))]"))} style={{ height: ROW_H }}>
      {periods.map((d) => {
        const c = r.cells.get(d)!;
        const tone = cellTone(c.pct, c.capacity, c.hasAbsence);
        const isSel = sel?.resourceId === r.resourceId && sel?.date === d && r.isPerson;
        const show = c.committed === 0 ? "" : `${Math.round(c.committed / 60)}`;
        return (
          <button
            key={d}
            type="button"
            onClick={() => (r.isPerson && r.resourceId ? setSel({ resourceId: r.resourceId, date: d }) : opts?.totals ? undefined : toggle(r.key))}
            className={cn("flex shrink-0 items-center justify-center border-r border-[var(--border-subtle)] text-[10px] font-semibold tabular-nums outline-none", isSel && "ring-2 ring-inset ring-[var(--accent)]")}
  ...
  return (
    <div>
          ...
          <div className="flex min-w-full align-top">
            ...
            <div className="relative min-w-0 flex-1">
              <div className="flex border-b border-[var(--border-strong)] bg-[var(--panel-subtle)]" style={{ height: HEADER_H }}>
                {periods.map((d) => { const pl = periodLabel(d, gran); return <span key={d} className={cn("flex flex-col items-center justify-center border-r border-[var(--border-subtle)] text-[length:var(--text-xs)] leading-none", pl.weekend && "bg-[color-mix(in_oklab,var(--panel-strong)_50%,var(--panel))]")} style={{ flex: `1 0 ${colW}px`, minWidth: colW }}><span className="font-semibold text-[var(--muted-strong)]">{pl.top}</span><span className="mt-0.5 text-[9px] text-[var(--muted-soft)]">{pl.sub}</span></span>; })}
              </div>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-[length:var(--text-xs)] text-[var(--muted)]">
        ...
        <span className="ml-auto text-[var(--muted-soft)]">Число — часы за период · цвет — % загрузки · ✓ — перегруз принят</span>
      </div>
    </div>
  );
}
