...
export function useCrm() {
  ...
  // Конструктор CRM: переименование/архивация/флаг «по умолчанию» воронки (PATCH — full-replace).
  const updatePipeline = useCallback((id: string, input: { name: string; sortOrder: number; description?: string | null; isDefault?: boolean; status?: CrmStatus }) => guard(async () => { const r = await client.updatePipeline(id, input); setData((d) => (d ? { ...d, pipelines: d.pipelines.map((x) => (x.id === id ? r.pipeline : x)) } : d)); }), [client, guard]);
  // Конструктор CRM: стадии воронки — создание и полное обновление (переименование/переупорядочивание/архивация).
  const createDealStage = useCallback((input: { name: string; sortOrder: number; pipelineId?: string | null; status?: CrmStatus }) => guard(async () => { const r = await client.createDealStage(input); setData((d) => (d ? { ...d, dealStages: [...d.dealStages, r.dealStage] } : d)); }), [client, guard]);
  const updateDealStage = useCallback((id: string, input: { name: string; sortOrder: number; status?: CrmStatus }) => guard(async () => { const r = await client.updateDealStage(id, input); setData((d) => (d ? { ...d, dealStages: d.dealStages.map((x) => (x.id === id ? r.dealStage : x)) } : d)); }), [client, guard]);
  // Конструктор CRM: типы проектов — создание и полное обновление.
  const createProjectType = useCallback((input: { name: string; description?: string | null; status?: CrmStatus }) => guard(async () => { const r = await client.createProjectType(input); setData((d) => (d ? { ...d, projectTypes: [...d.projectTypes, r.projectType] } : d)); }), [client, guard]);
  const updateProjectType = useCallback((id: string, input: { name: string; description?: string | null; status?: CrmStatus }) => guard(async () => { const r = await client.updateProjectType(id, input); setData((d) => (d ? { ...d, projectTypes: d.projectTypes.map((x) => (x.id === id ? r.projectType : x)) } : d)); }), [client, guard]);
  ...
  return { client, data, status, error, reload: load, moveStage, movePipeline, finalize, createOpportunity, createClient, updateClient, createContact, updateContact, createProduct, updateProduct, createPipeline, updatePipeline, createDealStage, updateDealStage, createProjectType, updateProjectType, createStageTransition, deleteStageTransition, updateOpportunity, checkFeasibility, activate, loadActivities, createComment, createTask, createFile, updateTaskStatus };
}
