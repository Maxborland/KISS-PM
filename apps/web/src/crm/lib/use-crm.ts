...
export function useCrm() {
  ...
  const createContact = useCallback((input: { clientId: string; name: string; email?: string | null; role?: string | null }) => guard(async () => { const r = await client.createContact(input); setData((d) => (d ? { ...d, contacts: [r.contact, ...d.contacts] } : d)); }), [client, guard]);
  const createProduct = useCallback((input: { name: string; unit: string; price: number; type?: "service" | "goods" }) => guard(async () => { const r = await client.createProduct(input); setData((d) => (d ? { ...d, products: [r.product, ...d.products] } : d)); }), [client, guard]);

  return { client, data, status, error, reload: load, moveStage, finalize, createOpportunity, createClient, createContact, createProduct };
}
