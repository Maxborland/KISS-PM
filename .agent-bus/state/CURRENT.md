...
- `node scripts/verify-requirements-matrix.mjs --allow-blocked docs/status/phase9-requirements-matrix.json` passes with P9-001/P9-005 API/domain evidence. Strict verification intentionally fails until P9 UI/product implementation and E2E-080..083 are complete.
...
- Next recommended step: claim `P9-006-closed-portfolio-trends-api-read-models` and expose closed portfolio/trends read models through API with permission states, tenant isolation, trend/insight readback, and no mutation URLs for read-only users.
