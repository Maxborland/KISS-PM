...
async function seedDemoProjectWork(db: KissPmDatabase, createdAt: Date): Promise<void> {
  await db.transaction(async (transaction) => {
    ...
    const demoTasks = [
      // Суммарные «фазы» (0 работы, без назначений — движок их не роллапит, геометрия summary считается во фронте из детей).
      {
        id: "task-demo-phase-prep",
        title: "Фаза 1 · Подготовка",
        description: "Суммарная фаза: ресурсная оценка, состав команды и описание.",
        status: "in_progress",
      ...
      {
        id: "task-demo-phase-delivery",
        title: "Фаза 2 · Проверка и сдача",
        description: "Суммарная фаза: ревью импорта Gantt и закрытие smoke-замечания.",
        status: "review",
      ...
      // Веха (0 работы, нулевая длительность — toGanttData рисует ромбом по эвристике leaf+0 работы).
      {
        id: "task-demo-milestone-mvp",
        title: "Веха · MVP принят",
        description: "Контрольная точка: согласованный объём MVP принят постановщиком.",
        status: "new",
      ...
      },
      {
        ...
        workMinutes: 1440,
        wbsCode: "1.1",
        parentTaskId: "task-demo-phase-prep",
        taskType: "fixed_work",
        effortDriven: true,
        plannedStartMinute: 0,
        plannedFinishMinute: 480,
      ...
      },
      {
        ...
        workMinutes: 120,
        wbsCode: "2.1",
        parentTaskId: "task-demo-phase-delivery",
        taskType: "fixed_units",
        effortDriven: false,
    ...
    for (const task of demoTasks) {
      await transaction
        ...
        .values({
          ...
          durationMinutes: task.workMinutes === 0 ? null : task.durationWorkingDays * 480,
      ...
      // Суммарные фазы и веха (0 работы) — это структура плана, не задачи исполнителя: без участников/активности, чтобы не засорять «Мою работу».
      if (task.workMinutes === 0) continue;
